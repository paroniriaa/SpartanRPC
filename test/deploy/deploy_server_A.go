...
func createServerAGo(port string, serviceList []any, serverChannel chan *server.Server, waitGroup *sync.WaitGroup) {
...
func createServerA(port string, serviceList []any, registryURL string) {
	log.Println("main -> createServer: RPC server initialization routine start...")
	listener, err := net.Listen("tcp", port)
	if err != nil {
		log.Fatal("main -> createServer error: RPC server network issue:", err)
...
func main() {
	...
	createServerA(":9001", serviceList, registryURL)
}
